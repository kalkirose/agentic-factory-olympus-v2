...
- **Seat environment.** At start the daemon reads the three host properties
  ...
  the start (ADR-0030).
- **Line endings** (ADR-0076). The harness reads the bytes CI reads. The two
  settings ride every harness git call and every clone. Every commit leaves the
  committed bytes in the tree. A project that declares no LF rule is refused at
  the door.
