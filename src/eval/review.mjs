...
function roleBlock({ review, window, last, paths }) {
  ...
  return [
    ...
    'Eight tripwire metrics read the same ledgers you are reading, and their ids',
    ...
    'recheck each count as a stage run of their own) and `record-write-time` (the',
    'mean wall clock of the record write in minutes, over the same five, which is',
    'what the one-record-at-a-time write costs).',
    'The command center derives twelve measures of the record tree from these',
    ...
    'the write wall clock; and the tree series, the `active` records per write',
    'with the supersessions, splits and merges behind them.',
