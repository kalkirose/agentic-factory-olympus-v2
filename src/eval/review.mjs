...
function roleBlock({ review, window, last, paths }) {
  ...
  return [
    ...
    'cites and the `unit` of the record it is about, and it is never advisory:',
    'the verifier answers it at every grade, and a confirmed one blocks. Read the',
    'window on four numbers: record findings raised, how many the verifier',
    'confirmed, the `reconcile-round` stamps they cost, and the residuals that',
    'went to tickets (`reconciliation-written` with `ok: false` and a `cause`, or',
    'with `partial: true`). Mostly confirmed says the rule is earning its rounds.',
    'Mostly refuted says the review seat is noisy about documents, and the answer',
    'is the record criteria and the brief.',
