...
function roleBlock({ review, window, last, paths }) {
  ...
  return [
    ...
    'Every `repair-round` carries the `cap` it counted against: the code cap for a',
    'diff that holds a code file, and `gates.reconcileRounds` for a diff of',
    'decision records alone. Read a `cap-exhausted` stall against the number on',
    'the round rather than against a constant.',
    'A finding about the shape of the implementation takes a repair round like',
    'any other finding and buys no pass of its own.',
    'A finding on a decision record carries `record: true` and the `criterion`',
    'it cites, and it is never advisory: the verifier answers it at every grade,',
    'and a confirmed one blocks. Read the window on four numbers: record findings',
    'raised, how many the verifier confirmed, the `repair-round` stamps carrying',
    '`phase: "reconcile"` they cost, and the residuals that went to tickets',
    '(`reconciliation-written` with `partial: true`, or `ok: false` with',
    '`cause: "record-layer-red"`). Mostly confirmed says the rule is earning its',
    'rounds. Mostly refuted says the review seat is noisy about documents, and',
    'the answer is the record criteria and the brief.',
    ...
    'Six tripwire metrics read the same ledgers you are reading, and their ids',
    ...
    'share of record findings the verifier refuted) and',
    '`reconcile-fallbacks-window` (ships whose in-run record rewrite ended in a',
    'fallback, over the ships judged owed).',
