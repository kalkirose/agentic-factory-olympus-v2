...
function roleBlock({ review, window, last, paths }) {
  ...
  return [
    ...
    'cites and the `unit` of the record it is about. A HIGH one goes to the',
    'verifier and a confirmed one blocks. One below HIGH is a remark: it carries',
    '`advisory: true`, it opens no round, the render it belongs to lists it under',
    '`advisory`, and the writer that is dispatched on its record for a HIGH is',
    'handed it. Read the window on five numbers: record HIGHs raised, how many',
    'the verifier confirmed, the `reconcile-round` stamps they cost, the residuals',
    'that went to tickets (`reconciliation-written` with `ok: false` and a `cause`,',
    'or with `partial: true`), and the remarks that shipped unanswered.',
    'Mostly confirmed says the rule is earning its rounds. Mostly refuted says the',
    'review seat is noisy about documents, and the answer is the record criteria',
    'and the brief. Read the shipped remarks by criterion and unit: a `truth`',
